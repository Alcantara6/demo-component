        ...
        <div id="todoList">
            ...
            <transition-group tag='ul' name='todoUl'>
                ...
                :key='todo.id'>
                      ...
                      <!-- :class={}-->
                      <span :class='{hasDone:todo.isDone}'>{{index+1}}. {{todo.title}}</span>
                      <span class='finishedTime' v-if='todo.isDone'>{{ todo.finishedTime }}</span>
                      <div class="todoItemBtn"> 
                          <button type='button' @click='todo.isDone=false'>{{ 'reset' }}</button>
            ...
            <p class='summarize' v-if='left'>共<strong>{{total}}</strong>件待办事项，<strong>{{finished}}</strong>件已完成，<strong>{{left}}</strong>件未完成</p>
        </div>
    </div>
</template>
...
export default {
  ...
  methods:{
      remove: function (index) {
        ...
        // this.nextTodoId--;     //不要再减，后面添加的id会与前面的重复
      },
